...
# Do not deploy publicly without authentication and stricter CORS settings.
CORS(app)
...
CONVERTED_DIR = os.path.join(VIDEOS_DIR, "converted")
...
def convert_video(timestamp):
    """Convert video to browser-friendly H.264 format"""
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'Conversion failed: {e.stderr.decode()}'
        }), 500
...
def delete_event(timestamp):
    """Delete event files and remove event from CSV"""
    try:
...
def open_folder(folder):
    """Open folder in file explorer for local review"""
    if folder == 'images':
        path = IMAGES_DIR
    elif folder == 'videos':
        path = VIDEOS_DIR
...
def find_thumbnail(timestamp, event_type):
    """Find thumbnail file by timestamp and event type"""
    patterns = [
...
def find_video(timestamp, event_type, filename):
    """Find video file by timestamp and event type"""
    if filename:
        converted_path = os.path.join(CONVERTED_DIR, filename)
        if os.path.exists(converted_path):
...
def create_sample_events():
    """
    Create fake sample events for testing.

    These use placeholder coordinates only.
    Do not commit real driver GPS coordinates, videos, images, or event logs.
    """
    return [
        {
...
if __name__ == '__main__':
    ...
    # Localhost only for safer GitHub/demo use.
